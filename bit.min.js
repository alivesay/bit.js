...
var BitObject={mixins:null,create:function(){var a=Object.create(this);this._construct.apply(a,arguments);return a},extend:function(a,b,c){var d=Object.create(this),e,f,g;b=b||{};if(a)for(f=Object.getOwnPropertyNames(a),g=f.length;g--;)e=f[g],b.hasOwnProperty(e)||(d[e]=a[e]);if(c)for(d.mixins=[],g=c.length;g--;){d.mixins.push(c[g]);if(c[g].mixins)for(a=c[g].mixins.length;a--;)-1===BitUtil.arrayIndexOf(d.mixins,c[g].mixins[a])&&d.mixins.push(c[g].mixins[a]);f=Object.getOwnPropertyNames(c[g]);for(a=
f.length;a--;)e=f[a],d.hasOwnProperty(e)||(d[e]=c[g][e])}if(this.mixins)for(g=this.mixins.length;g--;)-1===BitUtil.arrayIndexOf(d.mixins,this.mixins[g])&&d.mixins.push(this.mixins[g]);Object.defineProperties(d,b);return d},_construct:bit_noop,_constructMixin:function(a,b){-1!==BitUtil.arrayIndexOf(this.mixins,a)&&a._construct.apply(this,b)},_super:function(a,b,c){return Object.getPrototypeOf(a)[b].apply(this,c)}};bit.core.BitColor={};
var BitColor=new BitObject.extend({AMASK:BitUtil.isLittleEndian()?4278190080:255,BMASK:BitUtil.isLittleEndian()?16711680:65280,GMASK:BitUtil.isLittleEndian()?65280:16711680,RMASK:BitUtil.isLittleEndian()?255:4278190080,RGBMASK:BitUtil.isLittleEndian()?16777215:4294967040,ASHIFT:BitUtil.isLittleEndian()?24:0,BSHIFT:BitUtil.isLittleEndian()?16:8,GSHIFT:BitUtil.isLittleEndian()?8:16,RSHIFT:BitUtil.isLittleEndian()?0:24,getColor:function(a,b,c,d){return((void 0===d?255:d)<<this.ASHIFT|c<<this.BSHIFT|
b<<this.GSHIFT|a<<this.RSHIFT)>>>0},blendColors:function(a,b){var c=a>>this.RSHIFT&255,d=a>>this.GSHIFT&255,e=a>>this.BSHIFT&255,f=a>>this.ASHIFT&255,g=b>>this.RSHIFT&255,h=b>>this.GSHIFT&255,k=b>>this.BSHIFT&255;return 0===f?b:c*f+g*(255-f)>>8<<this.RSHIFT|d*f+h*(255-f)>>8<<this.GSHIFT|e*f+k*(255-f)>>8<<this.BSHIFT|f<<this.ASHIFT}});bit.core.BitBuffer={};
var BitBuffer=BitObject.extend({width:0,height:0,canvas:null,canvasCtx:null,data:null,dataLUT:null,_canvasCtxImageData:null,_buf:null,_buf8:null,_construct:function(a,b){this.width=a||this.width;this.height=b||this.height;this.dataLUT=this._buildLUT(this.width,this.height);this.canvas=document.createElement("canvas");this.canvas.id="bit_buffer_"+(new Date).getTime().toString();this.canvas.width=this.width;this.canvas.height=this.height;this.canvasCtx=this.canvas.getContext("2d");this.canvasCtx.imageSmoothingEnabled=
...
Math.max(0,a),g=Math.max(0,b);a=Math.min(this.width,a+c);b=Math.min(this.height,b+d);d=a;for(a=b;d-- >f;){for(;a-- >g;)this.data[this.dataLUT[d][a]]=BitColor.blendColors(e,this.data[this.dataLUT[d][a]])|this.AMASK;a=b}}});bit.core.BitDimensions={};var BitDimensions=BitObject.extend({width:0,height:0,_construct:function(a,b){this.width=a||this.width;this.height=b||this.height}});bit.core.BitEntityManager={};
var BitEntityManager=BitObject.extend({entities:null,addEntity:function(a,b){this.entities=this.entities||[];a.id=b||null;a.parent=this;this.entities.push(a)},removeEntity:function(a){this.entities&&(BitUtil.arrayRemove(this.entities,a),a.parent=null)},tick:function(a,b,c){for(var d=this.entities?this.entities.length:0;d--;)this.entities[d].tick(a,b,c)},render:function(a,b,c){for(var d=this.entities?this.entities.length:0;d--;)this.entities[d].render(a,b,c)},hasChildren:function(){return this.entities&&0<
this.entities.length}});bit.core.BitEvent={};var BitEvent=BitObject.extend({name:null,type:"BitEvent",notifier:null,_construct:function(a,b){this.name=a;this.notifier=b||null}},{qualifiedName:{get:function(){return this.type+"."+(this.name||"UNDEFINED")}}});bit.core.BitEventHandler={};var BitEventHandler=BitObject.extend({handleEvent:function(a){throw Error("BitEventHandler.handleEvent: No implementation for abstract method defined");}});bit.core.BitEntity={};var BitEntity=BitObject.extend({id:null,active:!0,parent:null},null,[BitEventHandler,BitEntityManager]);bit.core.BitEventNotifier={};var BitEventNotifier=BitObject.extend({_handlers:[],addHandler:function(a,b){var c=this._handlers[b]=this._handlers[event.qualifiedName]||[];-1===BitUtil.arrayIndexOf(c,b)&&c.push(a)},handleEvent:function(a){for(var b=this._handlers[a.qualifiedName],c=b?b.length:0,d;c--;)d=b[c],d.handleEvent(a)&&BitUtil.arrayRemove(b,a.qualifiedName)}},null,[BitEventHandler]);bit.core.BitApp={};

'use strict';var bit={core:{}};bit.core.bit_noop={};bit.core.BitUtil={};
var bit_namespace=function(a,b){var c=b.split("."),d=c.shift();if(void 0===a[d])a[d]={};else throw Error('bit_namespace: Namespace "'+d+'" already exists.');return 0<c.length?bit_namespace(a[d],c.join(".")):null},bit_noop=function(){},BitUtil={arrayIndexOf:function(a,b){var c,d=a.length;for(c=0;c<d;c++)if(a[c]===b)return c;return-1},arrayRemove:function(a,b){var c=this.arrayIndexOf(a,b);-1!==c&&a.splice(c,1)},isLittleEndian:function(){var a=new ArrayBuffer(4),b=new Uint8Array(a);(new Uint32Array(a))[0]=
3735928559;return 239===b[0]}};bit.core.BitObject={};
var BitObject={mixins:null,className:"BitObject",id:0,generateID:function(){return++BitObject.id},create:function(){var a=Object.create(this);a.id=this.generateID();this._construct.apply(a,arguments);return a},extend:function(a,b,c,d){var e=Object.create(this),f,g;c=c||{};e.className=a;if(b)for(f=Object.getOwnPropertyNames(b),g=f.length;g--;)a=f[g],c.hasOwnProperty(a)||(e[a]=b[a]);if(d)for(e.mixins=[],g=d.length;g--;){e.mixins.push(d[g]);if(d[g].mixins)for(b=d[g].mixins.length;b--;)-1===BitUtil.arrayIndexOf(e.mixins,
d[g].mixins[b])&&e.mixins.push(d[g].mixins[b]);f=Object.getOwnPropertyNames(d[g]);for(b=f.length;b--;)a=f[b],e.hasOwnProperty(a)||(e[a]=d[g][a])}if(this.mixins)for(g=this.mixins.length;g--;)-1===BitUtil.arrayIndexOf(e.mixins,this.mixins[g])&&e.mixins.push(this.mixins[g]);Object.defineProperties(e,c);return e},toString:function(){return"[ object "+this.className+" "+this.id+" ]"},_construct:bit_noop,_constructMixin:function(a,b){-1!==BitUtil.arrayIndexOf(this.mixins,a)&&a._construct.apply(this,b)},
_super:function(a,b,c){return Object.getPrototypeOf(a)[b].apply(this,c)}};bit.core.BitColor={};
...
var BitBuffer=BitObject.extend("BitBuffer",{width:0,height:0,data:null,dataLUT:null,_buffer:null,_buffer8:null,_construct:function(a,b){this.width=a||this.width;this.height=b||this.height;this.dataLUT=this._buildLUT(this.width,this.height);this._buffer=new ArrayBuffer(this.width*this.height*4);this._buffer8=new Uint8ClampedArray(this._buffer);this.data=new Uint32Array(this._buffer)},_buildLUT:function(a,b){var c,d,e=[];e.length=a;for(c=0;c<a;c++)for(e[c]=[],e[c].length=b,d=0;d<b;d++)e[c][d]=c+d*a;
return e},clear:function(a){for(var b=this.data.length;b--;)this.data[b]=a},putPixel:function(a,b,c,d,e,f){0>a||a>=this.width||0>b||b>=this.height||(this.data[this.dataLUT[a][b]]=f<<BitColor.ASHIFT|e<<BitColor.BSHIFT|d<<BitColor.GSHIFT|c<<BitColor.RSHIFT)},blit:function(a,b,c){var d=a.data,e=a.width;a=a.height;var f=e,g=a;b+=e;for(var h=c+a,k;f--;){for(b--;g--;)h--,0>b||0>h||b>=this.width||h>=this.height||(k=f+g*e,d[k]>>BitColor.ASHIFT&BitColor.AMASK&&(this.data[this.dataLUT[b][h]]=this.blendColors(d[k],
this.data[this.dataLUT[b][h]])|BitColor.AMASK));g=a;h=c+a}},blitNoAlpha:function(a,b,c){var d=a.width,e=a.height,f=d,g=e;b+=d;for(d=c+e;f--;){for(b--;g--;)d--,0>b||b>=this.width||0>d||d>=this.height||(this.data[this.dataLUT[b][d]]=a.data[a.dataLUT[f][g]]|BitColor.AMASK);g=e;d=c+e}},drawLine:function(a,b,c,d,e){for(var f=Math.abs(c-a),g=a<c?1:-1,h=Math.abs(d-b),k=b<d?1:-1,l=(f>h?f:-h)>>1,m=l;;){0<=a&&0<=b&&a<this.width&&b<this.height&&(this.data[this.dataLUT[a][b]]=e);if(a===c&&b===d)break;m>-f&&(l-=
h,a+=g);m<h&&(l+=f,b+=k)}},drawRect:function(a,b,c,d,e){},fillRect:function(a,b,c,d,e){var f=Math.max(0,a),g=Math.max(0,b);a=Math.min(this.width,a+c);b=Math.min(this.height,b+d);d=a;for(a=b;d-- >f;){for(;a-- >g;)this.data[this.dataLUT[d][a]]=BitColor.blendColors(e,this.data[this.dataLUT[d][a]])|BitColor.AMASK;a=b}}});bit.core.BitDimensions={};var BitDimensions=BitObject.extend("BitDimensions",{width:0,height:0,_construct:function(a,b){this.width=a||this.width;this.height=b||this.height}});bit.core.BitEntityManager={};
...
var BitScreen=BitObject.extend("BitScreen",{canvas:null,data:null,_canvasCtx:null,_canvasCtxImageData:null,_buffer:null,_buffer8:null,_construct:function(a,b){this._constructMixin(BitRectangle,[0,0,a,b]);this._constructMixin(BitEntityManager);this._constructMixin(BitBuffer);this.canvas=document.createElement("canvas");this.canvas.width=this.width;this.canvas.height=this.height;this._canvasCtx=this.canvas.getContext("2d");this._canvasCtxImageData=this._canvasCtx.getImageData(0,0,this.width,this.height);
this._buffer=new ArrayBuffer(this._canvasCtxImageData.data.length);this._buffer8=new Uint8ClampedArray(this._buffer);this.data=new Uint32Array(this._buffer)},tick:function(a,b){for(var c=this.entities?this.entities.length:0;c--;)this.entities[c].tick(a,b,this)},render:function(a,b){for(var c=0;c!==this.entities.length;)this.entities[c].render(a,b,this),c++;this._canvasCtxImageData.data.set(this._buffer8);this._canvasCtx.putImageData(this._canvasCtxImageData,0,0)}},null,[BitRectangle,BitEntityManager,
